# High-Throughput PyTorch Video Pipeline: From Disk I/O to Training
...
Training on high-dimensional data (like 3D Video Tensors) often suffers from the "Small File Problem," where disk I/O latency bottlenecks the GPU. This project solves that by engineering a **streaming data pipeline** using `WebDataset`. It transforms thousands of random-access files into sequential shards, enabling linear-speed streaming directly to the GPU VRAM.
...
* **Storage Strategy (Mock Data Lake):**
    > **Note on Infrastructure:** This MVP currently utilizes a local NVMe SSD to simulate a "Mock Data Lake" for rapid validation of the sharding logic. The pipeline is architected with `pathlib` and standard URI resolution, allowing for a seamless transition from local `file://` paths to **Azure Data Lake Storage (ADLS)** in the production phase without refactoring the core logic.
...
* **Function:** Generates synthetic 3D video tensors (Float16) and packages them into `.tar` shards.
* **Output:** 10 sequential shards containing 1,000 video samples + `metadata.parquet`.
...
* **Key Features:**
    ...
    * Real-time decoding of `.pth` and `.json` bytes.
...
* **Methodology:** Includes a high-repetition stress test (500 loops) to capture sustained GPU utilization metrics.

#### Phase 4: Integration (Model Training)
* **Script:** `training.py`
* **Function:** Validates data compatibility with a standard PyTorch Training Loop.
* **Model:** 3D-CNN (Conv3d) with custom dimension permutation to match channel-first requirements.
...
**Outcome Statement:**
> "By implementing a sharded WebDataset pipeline, I reduced I/O latency overhead significantly, increasing the data throughput of the RTX 4070 from **[INSERT NAIVE THROUGHPUT]** samples/sec to **[INSERT SHARDED THROUGHPUT]** samples/sec. This resulted in a **[INSERT SPEEDUP eg. 19x]** net increase in data loading speed."
